import { Settings as SettingsIcon, CreditCard, Shield, Bell } from "lucide-react";
import { motion } from "framer-motion";
import DashboardLayout from "@/components/DashboardLayout";

const plans = [
  { name: "Free", price: "$0", features: ["1 Bot", "5 Commands", "100 Users", "Basic Analytics"], current: false },
  { name: "Pro", price: "$19", features: ["10 Bots", "Unlimited Commands", "10K Users", "Full Analytics", "Priority Support"], current: true },
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input className="w-full bg-muted border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary" defaultValue="user@example.com" />
            </div>
            <div>
              <label className="text-xs text-muted-foreground block mb-1.5">Display Name</label>
              <input className="w-full bg-muted border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary" defaultValue="Bot Developer" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {plans.map((plan, i) => (
              <motion.div
                ...
                className={`glass-card p-5 ${plan.current ? "glow-border" : ""}`}
              >
                ...
                <button className={`mt-4 w-full px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  plan.current
